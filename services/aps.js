...
const { APS_CLIENT_ID, APS_CLIENT_SECRET, APS_BUCKET } = require('../config/forge.js');
...
service.deleteObject = async (objectName) => {
    await service.ensureBucketExists(APS_BUCKET);
    const resp = await new APS.ObjectsApi().deleteObject(APS_BUCKET, objectName, null, await service.getInternalToken());
    if(resp.statusCode === 404) throw Error(404)
};
